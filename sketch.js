...
function setup() {
  ...
  beesheildGroup = new Group()


  beesheild = createSprite(playerBee.x, playerBee.y);
    beesheild.scale = 0.2;
    
    beesheild.visible=false
    beesheild.addImage("beesheildImage", sheildImage);
    //beesheildGroup.add(beesheild);
...
function draw() {
  ...
  if (gameState === PLAY) {
    ...
    if(sheildGroup.isTouching(playerBee)){
      
      beesheild.visible=true
      //beesheild.velocityY = +(8 - 2 * score / 150);
      //sheildGroup.destroyEach()
    }
    if(beesheildGroup.isTouching(stonesGroup)||beesheildGroup.isTouching(ballsGroup)){
      //beesheildGroup.destroyEach()
    }
